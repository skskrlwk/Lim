...
public class ThreadMain {
	...
	public static void main(String[] args) {
		...
		/*
		  	개발자는 run() 메소드를 호출하는 것이 아니라 start() 메소드를 호출하여 사용한다.
		  	JVM이 알아서 Thread 의 run() 메소드를 호출시켜 준다.
		  	또한 JVM 의 스케줄러(Scheduler)가 알아서 스레드를 실행시켜주므로
		  	어떤 스레드가 먼저 실행되는지는 아무도 모른다.
		  	
		  	ThreadWho 클래스와 ThreadMyname 클래스가 스레드로 구현되었기 때문에 
		  	무한루프로 실행되지만 번갈아 가며 실행이 된다.
		  	하지만 각 스레드마다 실행되어지는 순서는 스케줄러가 알아서 실행시켜, 어떤 스레드가 실행되는지 예측불가하다.
		  	이것이 스레드의 특징이다.
		 */

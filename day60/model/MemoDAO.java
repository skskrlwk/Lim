...
public class MemoDAO implements InterMemoDAO{
	...
	*/
	...
	public List<HashMap<String, String>> getAllMemo() throws SQLException {
		...
		return memolist;
	}// end of List<HashMap<String, String>> getAllMemo() ---------------------------

	
	// *** Memo를 보여주는 메소드 *** //
	@Override
	public List<MemoVO> getAllMemoVO() throws SQLException {
		...
		try {
			...
			while(rs.next()) {
				...
				memovo.setWritedate(rs.getString("writedate"));
				memovo.setCip(rs.getString("cip"));
				memovo.setName(rs.getString("name"));
				memovo.setStatus(rs.getInt("status"));
	...
	// *** 전체 페이지 갯수 알아오는 메소드 *** 
	@Override
	public int getTotalCountMemo() throws SQLException {
		int result = 0;
	...
	// *** 전체 페이지 갯수 알아오는 메소드 *** 
		@Override
		public int getTotalCountMemoVO() throws SQLException {
			int result = 0;
	...
	public List<HashMap<String, String>> getAllMemo(int sizePerPage, int currentShowPageNo) throws SQLException {
		...
		try {
			...
			String sql = " SELECT idx, msg, writedate, cip, name, email, gender, age " 
						+" FROM " 
					    +" ( "  
					    +"  SELECT rownum AS RNO, idx, msg, writedate, " 
					    +"          cip, name, email, gender, age "  
					    +"  FROM " 
					    +"  ( "  
					    +"    SELECT A.idx, A.msg, "  
					    +"           TO_CHAR(A.writedate, 'yyyy-mm-dd hh24:mi:ss') AS writedate, " 
		...
		return memolist;
	}// end of List<HashMap<String, String>> getAllMemo(int sizePerPage, int currentShowPageNo)----------------------------


	@Override
	public List<MemoVO> getAllMemoVO(int sizePerPage, int currentShowPageNo) throws SQLException {
		List<MemoVO> memolist = null;
		...
		try {
			...
			String sql = " SELECT idx, msg, writedate, cip, name, email, gender, birthday, status " 
						+" FROM " 
					    +" ( "  
					    +"  SELECT rownum AS RNO, idx, msg, writedate, " 
					    +"          cip, name, email, gender, birthday, status "  
					    +"  FROM " 
					    +"  ( "  
					    +"    SELECT A.idx, A.msg, "  
					    +"           TO_CHAR(A.writedate, 'yyyy-mm-dd hh24:mi:ss') AS writedate, " 
					    +"           A.cip, B.name,	 B.email,  B.gender, B.birthday, A.status " 
					    +"    FROM jsp_memo A JOIN jsp_member B  "  
					    +"    ON A.userid = B.userid " 
				//	    +"    WHERE A.status = 1 "  
					    +"    ORDER BY idx DESC "  
		...
		return memolist;
	}// end of List<MemoVO> getAllMemoVO(int sizePerPage, int currentShowPageNo) ------------------------


	@Override
	public int deleteMemo(String str) throws SQLException {
		int result = 0;
		try {
			conn = ds.getConnection();
			// DBCP객체 ds를 통해 context.xml에서 이미 설정된 Connection 객체를 빌려오는 것이다.
	...
	@Override
	public int RollBackMemo(String str) throws SQLException {
		int result = 0;
		try {
			conn = ds.getConnection();
			// DBCP객체 ds를 통해 context.xml에서 이미 설정된 Connection 객체를 빌려오는 것이다.
